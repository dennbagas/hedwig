...
func TestHandleCallbackRerunError(t *testing.T) {
	...
	if tg.Sent[0].Params.Keyboard != nil {
		t.Errorf("keyboard = %+v, want nil (no keyboard option) so the retry button remains for another attempt", tg.Sent[0].Params.Keyboard)
	}
